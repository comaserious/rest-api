...
public class HateoasTestController {
    ...
    /* 필기.
    *   Hateoas 란?
    *   하나의 api 를 동작시켰을때, 그전 상황이나 그 후 상황에 대한 링크를
    *   제공함으로서 클라이언트가 리소스(자원)에 대한 이해를 더욱 충분히 할 수 있도록 링크를 제공하는것
    *   RestFul API 라고 한다.*/
    ...
    public ResponseEntity<ResponseMessage> findAllUsers(){
        ...
        /* EntityModel : 헤이티오스에서 제공하는 클래스로 rest api 를 만들 때 해당 리소스와 관련 된 링크를 포함할 수 있게 함*/
        ...
        responeMap.put("user",userWithRel);



        return ResponseEntity.ok().headers(headers).body(new ResponseMessage(200,"hateoas 전체 조회",responeMap));
    }
}
